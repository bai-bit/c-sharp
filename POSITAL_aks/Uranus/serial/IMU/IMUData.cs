...
namespace Uranus.Data
{
    public class IMUData
    {
        ...
        static float roll_temp;
        static float yaw;

        static float acc_x;
        static float acc_y;
        static float acc_z;
        static float acc_x_temp;
        static float acc_y_temp;
        static float acc_z_temp;

        static float gyr_x;
        static float gyr_y;
        static float gyr_z;
        static float gyr_x_temp;
        static float gyr_y_temp;
        static float gyr_z_temp;
        static public IMUData Decode(byte[] buf, int len)
        {
            ...
            while (offset < len)
            {
                ...
                {
                    case (byte)0:
                        // pitch   roll   acc_x   acc_y
                        pitch = (float)((Int16)(buf[2] << 8 | (buf[1]))) / 100;
                        roll = (float)((Int16)(buf[4] << 8 | (buf[3]))) / 100;
                        yaw = 0;

                        acc_x = (float)((Int16)(buf[6] << 8 | (buf[5]))) / 1000;
                        acc_y = (float)((Int16)(buf[8] << 8 | (buf[7]))) / 1000;
                        

                        if (pitch <= 90 && pitch >= -90 && roll >= -180 && roll <= 180 && acc_x <= 4 && acc_x >= -4 && acc_y <= 4 && acc_y >= -4 && acc_z <= 4 && acc_z >= -4 && gyr_x <= 500 && gyr_x >= -500 && gyr_y <= 500 && gyr_y >= -500 && gyr_z <= 500 && gyr_z >= -500)
                        {
                            imuData.SingleNode.Eul[1] = pitch;
                            imuData.SingleNode.Eul[0] = roll;
                            pitch_temp = pitch;
                            roll_temp = roll;
                            imuData.SingleNode.Eul[2] = yaw;

                            imuData.SingleNode.Acc[0] = acc_x;
                            imuData.SingleNode.Acc[1] = acc_y;
                            acc_x_temp = acc_x;
                            acc_y_temp = acc_y;
                            imuData.SingleNode.Acc[2] = acc_z;

                            imuData.SingleNode.Gyr[0] = gyr_x;
                            imuData.SingleNode.Gyr[1] = gyr_y;
                            imuData.SingleNode.Gyr[2] = gyr_z;
                        }
                        else
                        {
                            imuData.SingleNode.Eul[1] = pitch_temp;
                            imuData.SingleNode.Eul[0] = roll_temp;

                            imuData.SingleNode.Acc[0] = acc_x_temp;
                            imuData.SingleNode.Acc[1] = acc_y_temp;
                            imuData.SingleNode.Acc[2] = acc_z_temp;

                            imuData.SingleNode.Gyr[0] = gyr_x_temp;
                            imuData.SingleNode.Gyr[1] = gyr_y_temp;
                            imuData.SingleNode.Gyr[2] = gyr_z_temp;
                        }
                     ...
                     case (byte)1:
                        ...
                        acc_z = (float)((Int16)(buf[2] << 8 | (buf[1]))) / 1000;
                        gyr_x = (float)((Int16)(buf[4] << 8 | (buf[3]))) / 10;
                        gyr_y = (float)((Int16)(buf[6] << 8 | (buf[5]))) / 10;
                        gyr_z = (float)((Int16)(buf[8] << 8 | (buf[7]))) / 10;
                    ...
                    case (byte)ItemID.kItemRotationEular:
                        ...
                        imuData.SingleNode.Eul[1] = 10;
                        imuData.SingleNode.Eul[0] = 10;
                        imuData.SingleNode.Eul[2] = (float)((UInt16)(buf[1] << 8 | (buf[2]))) / 100;
